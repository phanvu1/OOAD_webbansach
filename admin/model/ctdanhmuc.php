<?php
    ...
    function searchCTDM($iddanhmuc, $keyword, $page, $limit) {
        $offset = ($page - 1) * $limit;
        $conn = connectdb();
        $keyword = "%" . $keyword . "%";
    
        // Đếm tổng số bản ghi khớp
        $countSql = "SELECT COUNT(*) FROM chitietdanhmuc 
                     WHERE iddanhmuc = :iddanhmuc 
                       AND (tenchitietdanhmuc LIKE :keyword 
                            OR idchitietdanhmuc LIKE :keyword)";
        $stmt = $conn->prepare($countSql);
        $stmt->bindValue(':iddanhmuc', $iddanhmuc, PDO::PARAM_INT);
