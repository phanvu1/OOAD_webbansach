...
function checkTrungEmailVaSDT($emailNV, $sdtNV) {
    ...
    $sqlEmail = "SELECT COUNT(*) FROM nhanvien WHERE email = :emailNV AND trangthai = 1";
    ...
    $sqlSDT = "SELECT COUNT(*) FROM nhanvien WHERE sodienthoai = :sdtNV AND trangthai = 1";
...
function getTenNhanVien($idnhanvien) {
    $conn = connectdb();
    if ($conn) {
        try {
            $sql = "SELECT idnhanvien FROM nhanvien WHERE idnhanvien = :idnhanvien";
            $stmt = $conn->prepare($sql);
            $stmt->bindParam(':idnhanvien', $idnhanvien, PDO::PARAM_INT);
            $stmt->execute();
            return $result = $stmt->fetch(PDO::FETCH_ASSOC)['ten'];
        } catch (PDOException $e) {
            error_log("Lỗi khi lấy tên nhân viên: " . $e->getMessage());
        }
    }
    return null;
}
...
function getAvailableNhanVien() {
    $conn = connectdb();
    if ($conn) {
        try {
            $sql = "SELECT idnhanvien FROM nhanvien WHERE trangthai = 1 AND chucvu = 'Nhân viên bán hàng' LIMIT 1";
            $sttm = $conn->prepare($sql);
            $sttm->execute();
            ...
            if ($result) {
                ...
                //updateNhanVienById($idnhanvien, 1);
                return $idnhanvien;
            }
        } catch (PDOException $e) {
